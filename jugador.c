...
void mostrarNombreArbol (arbolJugador* arbol, char nombre[])
{
    if (arbol)
    {
        mostrarNombreArbol (arbol->izq, nombre);
        if (strcmpi(arbol->dato.nombre,nombre)==0)
        {
            printf("\nJugadores: \n");
            mostrarUnStJugador(arbol->dato);
        }
        mostrarNombreArbol (arbol->der, nombre);
    }
}
...
void menuJugador(int validos, int dim)
{
    ...
    char validar[30];
    celdaDivision celd[dim];

    system("cls");
    printf("------> MENU DE JUGADORES\n\n");
    printf("[1] CARGAR JUGADOR/ES\n");
        ...
        }case 2:
        {
            ...
            case 4:{
                ...
                if (eleccion==1){
                    ...
                    validos = archivoToArrJug(celd, dim);
